...
Keypoint predictions are smoothed in real-time with an Exponential Moving Average.

Usage:
    python predict_videos_ema.py
    python predict_videos_ema.py --model runs/pose/dart_pose_overfit_all/weights/best.pt
    python predict_videos_ema.py --conf 0.5
    python predict_videos_ema.py --alpha 0.05
...
class EMAFilter:
    ...
    alpha: smoothing factor in (0, 1].
        Low  (0.05) = heavy smoothing, slow to react — good for static objects.
        High (0.5+) = light smoothing, fast to react.
    """
...
class KeypointSmoother:
    """Applies EMA filters per track ID per video stream.

    Filters are keyed by ByteTrack ID so that reordered detections don't
    swap smoothed positions. Tracks unseen for `reset_after` consecutive
    frames are pruned so a reappearing dart snaps to its new position.
    """

    def __init__(self, alpha=0.1, reset_after=5):
        self.alpha = alpha
        self.reset_after = reset_after
        # {track_id: [EMAFilter x4]}  — created on demand
        self.filters = {}
        # {track_id: frames_since_last_seen}
        self.miss_counts = {}

    def _get_filters(self, track_id):
        if track_id not in self.filters:
            self.filters[track_id] = [EMAFilter(self.alpha) for _ in range(4)]
            self.miss_counts[track_id] = 0
        return self.filters[track_id]

    def smooth(self, keypoints_data, track_ids):
        """Smooth keypoints tensor (N, 2, 3) using track IDs for correct mapping.

        track_ids: tensor/list of integer IDs from result.boxes.id, or None
                   if tracker lost all objects (falls back to index-based).
        Returns a new smoothed clone.
        """
        ...
        for i in range(len(data)):
            # Use track ID if available, otherwise fall back to index
            ...
            fx_tip_x, fx_tip_y, fx_base_x, fx_base_y = self._get_filters(tid)

            self.miss_counts[tid] = 0
            if tip_c > 0.3:
                data[i][0][0] = fx_tip_x(tip_x)
                data[i][0][1] = fx_tip_y(tip_y)
            if base_c > 0.3:
                data[i][1][0] = fx_base_x(base_x)
                data[i][1][1] = fx_base_y(base_y)

        # Increment miss count for unseen tracks and prune stale ones
        stale = []
        for tid in self.miss_counts:
            if tid not in seen_ids:
                self.miss_counts[tid] += 1
                if self.miss_counts[tid] >= self.reset_after:
                    stale.append(tid)
        for tid in stale:
            del self.filters[tid]
            del self.miss_counts[tid]
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Run dart pose on 3 videos simultaneously (EMA smoothing)")
    ...
    parser.add_argument("--alpha", type=float, default=0.05, help="EMA smoothing factor (lower = smoother, 0.01-0.5)")
    parser.add_argument("--reset-after", type=int, default=5, help="Reset EMA after N consecutive missed frames")
    ...
    smoothers = [KeypointSmoother(alpha=args.alpha, reset_after=args.reset_after) for _ in VIDEOS]
    ...
    while True:
        ...
        if need_update:
                ...
                if prev_frames[vid_idx] is not None and np.array_equal(frame, prev_frames[vid_idx]):
                    ...
                    for result in results:
                        if result.keypoints is not None:
                            ...
                            result.keypoints.data = smoothers[vid_idx].smooth(result.keypoints.data, track_ids)
            ...
            hud = f"{status} | conf={args.conf} iou={args.iou} alpha={args.alpha}"
            if has_ref:
