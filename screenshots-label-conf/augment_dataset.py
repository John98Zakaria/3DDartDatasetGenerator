...
def filter_oob_labels(rows: list[list[float]]) -> list[list[float]]:
    """Drop rows where any coordinate is outside [0, 1]."""
    out = []
    for row in rows:
        oob = False
        for i in range(1, 5):
        ...
        if not oob:
            for i in range(5, len(row), 3):
                if row[i + 2] > 0 and (row[i] < 0.0 or row[i] > 1.0 or row[i + 1] < 0.0 or row[i + 1] > 1.0):
                    oob = True
                    break
        if not oob:
            out.append(row)
...
def aug_salt_pepper(img: np.ndarray) -> np.ndarray:
    """Salt-and-pepper noise — random hot/dead pixels like camera static."""
    out = img.copy()
